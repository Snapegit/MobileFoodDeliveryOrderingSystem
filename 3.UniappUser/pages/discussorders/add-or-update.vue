	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussorders`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussorders`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussorders`, this.ruleForm);
						}else{
							await this.$api.add(`discussorders`, this.ruleForm);
						}
						uni.setStorageSync('discussordersCleanType',true);
