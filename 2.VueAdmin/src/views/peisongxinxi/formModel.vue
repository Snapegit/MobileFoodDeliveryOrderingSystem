				...
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="手机号" prop="shoujihao">
							<el-input class="list_inp" v-model="form.shoujihao" placeholder="手机号"
								 type="text" 								:readonly="!isAdd||disabledForm.shoujihao?true:false" />
						</el-form-item>
					</el-col>
					...
					<el-col :span="12">
						<el-form-item label="收货状态" prop="shouhuozhuangtai">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.shouhuozhuangtai?true:false"
								v-model="form.shouhuozhuangtai" 
								placeholder="请选择收货状态"
								>
								<el-option v-for="(item,index) in shouhuozhuangtaiLists" :label="item"
									:value="item"
									>
								</el-option>
							</el-select>
...
<script setup>
	...
	const type = ref('')
	//收货状态列表
	const shouhuozhuangtaiLists = ref([])
	//methods
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='shoujihao'){
					form.value.shoujihao = row[x];
					disabledForm.value.shoujihao = true;
					continue;
				}
				if(x=='peisongshijian'){
				...
				if(x=='shouhuozhuangtai'){
					form.value.shouhuozhuangtai = row[x];
					disabledForm.value.shouhuozhuangtai = true;
					continue;
				}
				if(x=='dizhi'){
			...
			form.value.shouhuozhuangtai='已确认'
			formVisible.value = true
		}
		...
		context?.$http({
			...
			if(json.hasOwnProperty('shoujihao')&& context?.$toolUtil.storageGet("role")!="管理员"){
				form.value.shoujihao = json.shoujihao
				disabledForm.value.shoujihao = true;
			}
		...
		shouhuozhuangtaiLists.value = "已确认,未确认".split(',')
	}
	//初始化
	//声明父级调用
	defineExpose({
