		...
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="12">
						<el-form-item label="菜色偏好" prop="caisepianhao">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.caisepianhao?true:false"
								v-model="form.caisepianhao" 
								placeholder="请选择菜色偏好"
								>
								<el-option v-for="(item,index) in caisepianhaoLists" :label="item"
									:value="item"
									>
								</el-option>
							</el-select>
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='caisepianhao'){
					form.value.caisepianhao = row[x];
					disabledForm.value.caisepianhao = true;
					continue;
				}
				if(x=='money'){
		...
		caisepianhaoLists.value = "酸甜,香辣,咸鲜,清淡,麻辣".split(',')
	}
	//初始化
	//声明父级调用
	defineExpose({
